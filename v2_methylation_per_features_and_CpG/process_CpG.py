...
def get_summary_for_all(CPG_dic):
    ...
    output_file=open("process_CPG_output/"+f+"_INsup_sorted_islands", "w") #CPG/process_CPG_output/"+f+"_INsup_sorted_islands", "w")
    ...
    output_file.close()
    output_file=open("process_CPG_output/"+f+"_OUTsup_sorted_islands", "w")  #CPG/process_CPG_output/"+f+"_OUTsup_sorted_islands", "w")
    output_file.write("\nISLANDS WITH PROBin < PROBout\n-island\tcountCPGin\tprobCmGin\tcountCPGout\tprobCPGout\tdiff\n")
    for entry in new_CPG_islandminus:
        for l in entry:
            output_file.write(l[0]+"\t"+str(l[1][0])+"\t"+str(l[1][2])+"\t"+str(l[2][0])+"\t"+str(l[2][2])+"\t"+str(round(l[1][2] - l[2][2],2))+"\n")
    output_file.close()
    output_file=open("process_CPG_output/"+f+"_INeqOUT_sorted_islands", "w") #Cprocess_CPG_output/"+f+"_INeqOUT_sorted_islands", "w")  PG/process_CPG_output/"+f+"_INeqOUT_sorted_islands", "w")
    output_file.write("\nISLANDS WITH PROBin = PROBout\n-island\tcountCPGin\tprobCmGin\tcountCPGout\tprobCPGout\tdiff\n")
